...
/**
 自定义排序用方法
 */
export function sortColumn(that,column) {

  // console.log("数据字段："+column.prop)
  // console.log("排序方式："+column.order)
  var fieldName = column.prop // 传输的列名
  fieldName = fieldName.replace(/([A-Z])/g,"_$1").toLowerCase() // 驼峰逆向转换 需要使用该方法逆向转换回对应查询列（可能部分数据不需要驼峰逆向转换需要单独处理）
  var sortingType = column.order // 传输的排序方式
  if(sortingType != null && sortingType != "") { // 为空则说明本次点击为取消单列排序
    if(sortingType == 'ascending') { // 正序排列
      sortingType = "ASC"  // 转换为数据库可识别模式
    } else if (sortingType == 'descending') { // 倒序排列
      sortingType = "DESC" // 转换为数据库可识别模式
    } else {
      sortingType = "" // 赋空使后台不执行排序逻辑
    }
    that.queryParams.fieldName = fieldName // 存放到传输字段中
    that.queryParams.sortingType = sortingType  // 存放到传输字段中
  } else {
    that.queryParams.fieldName = ""  // 若取消排序则清空传输字段
    that.queryParams.sortingType = "" // 若取消排序则清空传输字段
  }
  // console.log("传输参数："+JSON.stringify(that.queryParams))
  that.getList()
}
/**
 自定义排序用方法部分参数不进行驼峰转换版本
 */
export function sortColumnNoToLowerCase(that,column,list) {
  // console.log("数据字段："+column.prop)
  // console.log("排序方式："+column.order)
  var fieldName = column.prop // 传输的列名
  var flag = true
