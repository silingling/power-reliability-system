...
export default {
  ...
  computed: {
    ...mapGetters(['allAuth', 'crm', 'project']),
    permissionSave() {
      return this.allAuth && this.allAuth.manage && this.allAuth.manage.notice && this.allAuth.manage.notice.save
    },
    title() {
      if(this.isNotice){
        return this.unreadNums.announceCount > 0 ? `公告（${this.unreadNums.announceCount}）` : '公告'
      }else{
    ...
    menuList() {
      ...
      if ((this.allAuth.taskExamine) || this.project) {
        menuList.push({
          name: '任务',
          label: 1,
          countKey: 'taskCount'
    ...
    currentMenu() {
      ...
      // console.log("=this.menuLabel"+JSON.stringify(this.menuLabel))
      // console.log("=this.menuList"+JSON.stringify(this.menuList))
      return this.menuList.find(item => {
        // console.log(" item.label === this.menuLabel="+( item.label === this.menuLabel))
